...
const CONFIG = {
    runDurationMin:            355,   // 5h55min — fills the 6h cron window, 5min buffer before timeout
    ...
    // Cap items per cycle: ok->ok = sched_avg(22.5s) + cycle_time/2
    // With 10 items x 3.04s: ok->ok = 22.5 + 15 = 37.5s < 56s (min revert)
    maxItemsPerCycle:          10,

    // After N consecutive btn-not-found -> structural cooldown (stop wasting cycles)
    btnErrCooldownAt:          12,
    ...
    scheduler: {
        // [15s, 30s]: sched_avg=22.5s
        //   ok→ok = 22.5 + (maxItemsPerCycle x 3.04s)/2 = 22.5+15 = 37.5s
        //   37.5s < 56s (min revert) => arrive BEFORE revert on every cycle
        ok:               [15_000,    30_000],
        stable:           [15_000,    30_000],
...
function scheduleNext(item, status, message) {
    ...
    if (message.startsWith('buttons not found')) {
        st.btnErrStreak++;
        st.cooldown  = false;
        // After btnErrCooldownAt consecutive structural errors -> long sleep
        if (st.btnErrStreak >= CONFIG.btnErrCooldownAt) {
            st.nextCheck = now + rand(...CONFIG.scheduler.cooldown);
            console.warn('btn structural cooldown I#' + item.num + ' streak:' + st.btnErrStreak);
        } else {
...
async function runCycle(page, cycleNum) {
    ...
    const selected   = ready.slice(0, CONFIG.maxItemsPerCycle);
