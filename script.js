...
// Compiled once in main() after env vars are loaded — never rebuilt per-visit.
...
// Save button is always in-viewport after both dropdowns are selected.
// No scroll or boundingBox needed — light settle only.
...
async function createBrowser(cookies) {
    ...
    // Images + fonts + analytics — all blocked. Fonts add 100–400ms per load with no value.
...
(async () => {
    ...
    // Compile once after env vars are loaded — used on every item visit.
