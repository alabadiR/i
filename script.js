...
const CONFIG = {
    ...
    // How many consecutive btn-not-found failures before switching load strategy.
    // Visits 1..N use 'load' (full JS render). Visits > N use domcontentloaded (fast).
...
function initItemState(item) {
    itemState[item.num] = {
        ...
        totalFail:    0,
        cooldown:     false,
        btnErrStreak: 0,   // consecutive "buttons not found" — drives load strategy
    };
}
...
function scheduleNext(item, status, message) {
    ...
    // Ad is already in correct state — no penalty, same window as ok.
    ...
    // Page loaded with < 2 buttons — transient or structural issue.
    // Track btnErrStreak to drive the adaptive load strategy in processItem.
    // No failStreak penalty — this is NOT a form failure.
...
function printStartupSummary() {
    ...
    console.log(`     BtnLoad≤  : ${CONFIG.btnErrLoadThreshold} errors → use waitUntil:'load'`);
    console.log(bar + '\n');
}
...
function printStateSnapshot() {
    ...
    console.log(sep);
    console.log(`  ${'ID'.padEnd(5)} ${'Next'.padEnd(8)} ${'Streak'.padEnd(8)} ${'Total'.padEnd(7)} ${'BtnErr'.padEnd(7)} State`);
    console.log(sep);
    for (const it of CONFIG.items) {
        ...
        console.log(
            ...
            ` ${String(st.totalFail).padStart(3).padEnd(7)}` +
            ` ${String(st.btnErrStreak).padStart(3).padEnd(7)} ${state}`
        );
    }
    console.log(sep + '\n');
}
...
async function checkSession(page, label = 'general') {
    ...
    for (let attempt = 1; attempt <= CONFIG.cookieRetries; attempt++) {
        try {
            await page.goto(probeItem.url, { waitUntil: 'domcontentloaded', timeout: 30_000 });
...
async function processItem(page, item) {
    try {
        ...
        //   btnErrStreak 0     → 'domcontentloaded' (fast, first attempt)
        //   btnErrStreak 1..N  → 'load' (full JS render, trying to unlock the second button)
        //   btnErrStreak > N   → 'domcontentloaded' (confirmed structural, stop wasting time)
        const useFull = st.btnErrStreak >= 1 && st.btnErrStreak <= CONFIG.btnErrLoadThreshold;
        const loadStrat = useFull ? 'load' : 'domcontentloaded';
        if (useFull) console.log(`   🔄 load:'load' (btnErrStreak:${st.btnErrStreak})`);

        await page.goto(item.url, { waitUntil: loadStrat, timeout: 60_000 });
        await sleepRand(CONFIG.delays.pageLoad);

        // Wait for at least one listbox button to be present.
        await page.waitForSelector(CONFIG.selectors.listboxBtn, { timeout: 15_000 });

        // Poll until both buttons are present.
        // Timeout reduced to 3s (was 10s) — items that structurally have only 1 button
        // never get a second one, so the 10s timeout was wasting ~70s per cycle.
        ...
        if (btnCount < 2) {
            // On the FIRST failure for this item, save a screenshot for manual inspection.
            // This helps diagnose whether the 9 stuck items have a different page structure.
            if (st.btnErrStreak === 0) {
                try {
                    const shotPath = path.join(CONFIG.logsDir, `btn-${BATCH_DATE}-I${item.num}.png`);
                    await page.screenshot({ path: shotPath, fullPage: true });
                    console.log(`📸 First btn-err screenshot: ${path.basename(shotPath)}`);
                } catch {}
            }
            await sleepRand(CONFIG.delays.beforeSkip);
            console.log(`⚠️  ${itemLabel(item)} - buttons not found (${btnCount}) [btnErrStreak:${st.btnErrStreak}]`);
...
(async () => {
    ...
    while (Date.now() < RUN_END) {
        ...
        // Only check session on real failures (not stable, not btn-error which are structural).
        ...
        if (hasRealFailures) {
            console.warn(`\n⚠️  Cycle ${cycleNum}: 0 ok, ${cycleStats.pass - cycleStats.stable} real fail → checking session...`);
