...
// Common referers — real browsers almost always carry one.
// Empty string = direct navigation (~15% of real traffic).
const REFERER_POOL = [
...
const CONFIG = {
    ...
    delays: {
        pageLoad:        [1200, 2000], // reduced from [2000,3000] — waitForSelector catches slow pages
    ...
    scheduler: {
        ...
        // noReadyItemWait removed — Smart Sleep computes exact wait from item schedules
    },

    // Time-based execution: run for this many minutes per GitHub Actions job.
    // Set to 330 to stay within the 360-minute timeout with a 30-minute safety buffer.
    runDurationMin:            330,
...
// Short label for console/log — sequence number only.
function itemLabel(item) {
    return `I#${item.num}`;
}

// ETA — time remaining until the run's scheduled end (RUN_END set in main).
// Falls back to cycle-based estimate before RUN_END is defined.
let RUN_END = null;
function calcETA() {
    if (RUN_END) {
    ...
    // RUN_END not set yet — estimate from runDurationMin
...
async function runCycle(page, cycleNum) {
    ...
    if (ready.length === 0) {
        // SMART SLEEP: compute exactly when the nearest item becomes eligible.
        // Cap at (RUN_END - 15s) so we always have time for a graceful shutdown.
        // Minimum of 2s prevents degenerate spin loops.
...
(async () => {
    ...
    // Set the wall-clock end time for this run.
    // runDurationMin (330) leaves a 30-min safety buffer before GitHub's 360-min timeout.
    ...
    while (Date.now() < RUN_END) {
        ...
        if (cycleNum > 1 && cycleNum >= nextReset) {
            // Only reset if there are items about to be processed.
            // Resetting when all items are in cooldown wastes ~30s for zero benefit.
            ...
            if (hasReadyItems) {
                ...
                // Defer the reset — don't count current cycle toward the reset interval
                nextReset++;
            }
        }
        ...
        if (!cycleStats.noItems && cycleStats.i > 0 && cycleStats.ok === 0) {
            console.warn(`\n⚠️  Cycle ${cycleNum}: ${cycleStats.i} items processed, 0 ok → checking session...`);
        ...
        // Inter-cycle delay only after real work (skip when noItems — Smart Sleep already waited).
    ...
    // Final log flush — send whatever partial log has accumulated since the last scheduled send.
    // Guard against cycleNum=0 (run ended before any cycle completed — e.g. very long warm-up).
    if (cycleNum > 0) {
